import type { ResidentGeometryBinding } from '@render/backend/shared/contracts/IRenderBackend'
import { TerrainClusterArena } from '@render/backend/webgl2/terrain/runtime/TerrainClusterArena'
import { type TerrainUploadPacket } from '@render/backend/webgl2/terrain/runtime/TerrainUploadPacketBuilder'
import type {
  TerrainResidentRebuildStats,
  TerrainResidentRuntimeStats,
  TerrainResidentUploadExecutionBackend,
  TerrainResidentUploadStats,
} from '@render/backend/webgl2/terrain/runtime/TerrainResidentUploadExecutor'
import type {
  TerrainClusterUploadPlan,
  TerrainItemUploadPlan,
} from '@render/backend/webgl2/terrain/runtime/TerrainUploadPlanner'
import { terrainSectionKeyToString, type TerrainItem } from '@render/terrain/contracts/types'
import { WebGPUTerrainClusterBuffer } from './WebGPUTerrainClusterBuffer'
...
export class WebGPUTerrainResidentUploadExecutionBackend
  implements TerrainResidentUploadExecutionBackend
{
  private readonly clusterBuffers = new Map<string, WebGPUTerrainClusterBuffer>()
  ...
  private getOrCreateClusterItemBuffer(clusterKey: string, item: TerrainItem) {
    ...
    if (!buffer) {
      buffer = new WebGPUTerrainClusterBuffer(this.device, clusterKey, item)
  ...
  private rebaseVertexBytesForCluster(
    chunkX: number,
    chunkZ: number,
    cluster: Pick<TerrainClusterUploadPlan, 'cluster'>,
    vertexStride: number,
    vertexBytes: Uint8Array,
  ) {
    if (vertexStride <= 0 || vertexBytes.byteLength === 0) {
      return vertexBytes
    }

    const localChunkX = chunkX - cluster.cluster.clusterX * 4
    const localChunkZ = chunkZ - cluster.cluster.clusterZ * 4
    if (localChunkX === 0 && localChunkZ === 0) {
      return vertexBytes
    }

    const rebased = vertexBytes.slice()
    const vertexCount = Math.floor(rebased.byteLength / vertexStride)
    const words = new Uint32Array(rebased.buffer, rebased.byteOffset, rebased.byteLength / 4)
    const xOffset = localChunkX * 16 * 32
    const zOffset = localChunkZ * 16 * 32
    for (let vertexIndex = 0; vertexIndex < vertexCount; vertexIndex++) {
      const wordBase = vertexIndex * (vertexStride / 4)
      words[wordBase] += xOffset
      words[wordBase + 2] += zOffset
    }
    return rebased
  }
}
