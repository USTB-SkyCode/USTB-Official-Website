...
function buildIndexEntry(entry: CatalogEntry, defaultKey: string): IndexEntry {
...
function writePackIndex(entries: IndexEntry[], defaultKey: string) {
...
export function syncResourcePackArtifacts(): void {
  ...
  const indexEntries = catalog.entries.map(entry =>
    buildIndexEntry(entry, catalog.defaultKey),
  );

  writePackIndex(indexEntries, catalog.defaultKey);
}
